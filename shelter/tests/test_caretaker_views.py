...
class PublicCaretakerTests(TestCase):
    def setUp(self) -> None:
        self.caretaker = get_user_model().objects.create_user("test", "password1234")
    ...
    def test_login_required_caretaker_detail(self) -> None:
        response = self.client.get(
            reverse("shelter:caretaker-detail", kwargs={"pk": self.caretaker.pk})
        )
    ...
    def test_login_required_to_update_caretaker(self) -> None:
        response = self.client.get(
            reverse("shelter:caretaker-update", kwargs={"pk": self.caretaker.pk})
        )
    ...
    def test_login_required_to_delete_caretaker(self) -> None:
        response = self.client.get(
            reverse("shelter:caretaker-delete", kwargs={"pk": self.caretaker.pk})
        )
    ...
    def test_successful_caretaker_creation(self) -> None:
        ...
        response = self.client.post(
            reverse("shelter:caretaker-create"), data=data, follow=True
        )
...
class PrivatecaretakerTests(TestCase):
    ...
    def test_logined_user_has_access_to_detail_caretaker(self) -> None:
        response = self.client.get(
            reverse("shelter:caretaker-detail", kwargs={"pk": self.user.pk})
        )
    ...
    def test_logined_user_has_access_to_update_caretaker(self) -> None:
        response = self.client.get(
            reverse("shelter:caretaker-update", kwargs={"pk": self.user.pk})
        )
    ...
    def test_logined_user_has_access_to_delete_caretaker(self) -> None:
        response = self.client.get(
            reverse("shelter:caretaker-delete", kwargs={"pk": self.user.pk})
        )
    ...
    def test_update_anonymous_user_redirect(self) -> None:
        ...
        self.assertRedirects(
            response, "/accounts/login/?next=/shelter/caretakers/1/update/"
        )
    ...
    def test_successful_caretaker_deletion(self) -> None:
        caretaker1 = get_user_model().objects.create_user("Caretaker1", "password1234@")
        response = self.client.post(
            reverse("shelter:caretaker-delete", kwargs={"pk": caretaker1.pk}),
            follow=True,
        )
    ...
    def test_delete_caretaker_anonymous_user_redirect(self) -> None:
        ...
        self.assertEqual(response.status_code, 302)
        self.assertRedirects(
            response, "/accounts/login/?next=/shelter/caretakers/1/delete/"
        )
