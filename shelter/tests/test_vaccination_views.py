...
class PublicVaccinationTests(TestCase):
    def setUp(self) -> None:
        ...
        self.vaccination = Vaccination.objects.create(
            dog=self.dog, vaccine=self.vaccine, vaccination_date="2023-07-26"
        )

    def test_login_required_to_create_vaccination(self) -> None:
        response = self.client.get(
            reverse("shelter:vaccination-create", kwargs={"dog_id": self.dog.id})
        )

        self.assertNotEqual(response.status_code, 200)
        self.assertEqual(response.status_code, 302)
        self.assertRedirects(
            response, "/accounts/login/?next=/shelter/vaccination/create/1/"
        )

    def test_login_required_to_update_vaccination(self) -> None:
        response = self.client.get(
            reverse(
                "shelter:vaccination-update",
                kwargs={"dog_id": self.dog.id, "pk": self.vaccination.id},
            )
        )
        ...
        self.assertEqual(response.status_code, 302)
        self.assertRedirects(
            response, "/accounts/login/?next=/shelter/vaccination/update/1/1/"
        )

    def test_login_required_to_delete_vaccination(self) -> None:
        response = self.client.get(
            reverse(
                "shelter:vaccination-delete",
                kwargs={"dog_id": self.dog.id, "pk": self.vaccination.id},
            )
        )
        ...
        self.assertEqual(response.status_code, 302)
        self.assertRedirects(
            response, "/accounts/login/?next=/shelter/vaccination/delete/1/1/"
        )
...
class PrivateVaccinationTest(TestCase):
    ...
    def test_successful_vaccination_creation(self) -> None:
        data = {"vaccination_date": "2023-07-01", "vaccine": self.vaccine.id}
        ...
        self.assertTrue(
            Vaccination.objects.filter(vaccination_date="2023-07-01").exists()
        )

    def test_unsuccessful_vaccination_creation(self) -> None:
        data = {"vaccination_date": "", "vaccine": self.vaccine.id}
        response = self.client.post(
            reverse("shelter:vaccination-create", kwargs={"dog_id": self.dog.id}),
            data=data,
            follow=True,
    ...
    def test_successful_vaccination_deletion(self) -> None:
        vaccination1 = Vaccination.objects.create(
            dog=self.dog, vaccine=self.vaccine, vaccination_date="2023-06-26"
        )
        response = self.client.post(
            reverse(
                "shelter:vaccination-delete",
                kwargs={"dog_id": self.dog.id, "pk": vaccination1.id},
            )
        )
        ...
        self.assertFalse(
            Vaccination.objects.filter(vaccination_date=expected_date).exists()
        )
